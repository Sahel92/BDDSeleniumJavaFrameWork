...
public class CommonUtility extends BaseSetup {
	...
	/*
	 * get visible text of an element (not hidden by CSS)
	 * if element has long load time, use this method
	 */
...
public WebElement fluentWaitForElement(WebElement element, int timeoutSec, int pollingSec) {
	FluentWait<WebDriver> fWait = new FluentWait<WebDriver>(getDriver()).withTimeout(Duration.ofSeconds(30)).pollingEvery(Duration.ofSeconds(30)).ignoring(NoSuchElementException.class, TimeoutException.class).ignoring(StaleElementReferenceException.class);
	for(int i = 0; i <2; i++)
		fWait.until(ExpectedConditions.visibilityOf(element));
	return element;
}
// swtiches to specificed window using pageTtitle as parameter 
